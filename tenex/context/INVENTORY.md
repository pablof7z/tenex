...
*   **Native Function Calling Abstraction**: The architecture uses native LLM function calling. The `ToolPlugin` class serves as an adapter, allowing a single internal `Tool` definition to be used seamlessly across different LLM providers (Anthropic, OpenAI, Google) that support function calling via the `multi-llm-ts` library.
