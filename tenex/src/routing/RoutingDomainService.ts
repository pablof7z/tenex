...
/**
 * Domain service that encapsulates routing business logic
...
export class RoutingDomainService {
   ...
   */
  static canTransitionPhase(currentPhase: Phase, targetPhase: Phase): boolean {
    const validTransitions: Record<Phase, Phase[]> = {
      ...
      review: ["execute", "chat"], // Can go back to execute for fixes or chat for new requirements
    };
   ...
   */
  static validateRoutingDecision(
    ...
    if (decision.phase !== conversation.phase) {
      if (!RoutingDomainService.canTransitionPhase(conversation.phase, decision.phase)) {
        return {
          valid: false,
   ...
   */
  static inferPhaseFromContent(message: string): Phase | null {
   ...
   */
  static getDefaultAgentForPhase(phase: Phase, agents: Agent[]): Agent | null {
  ...
  static applyBusinessRules(
    ...
    if (!enhancedDecision.nextAgent && enhancedDecision.phase !== "chat") {
      const defaultAgent = RoutingDomainService.getDefaultAgentForPhase(
        enhancedDecision.phase,
        availableAgents
      );
      if (defaultAgent) {
   ...
   */
  static meetsPhaseTransitionCriteria(
    conversation: ConversationState,
    targetPhase: Phase
