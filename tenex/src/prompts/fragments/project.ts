...
// Project information fragment for routing
interface ProjectInformationArgs {
    name: string;
    description?: string;
    repository?: string;
    tags?: string[];
...
const validateProjectInformationArgs = (args: unknown): args is ProjectInformationArgs => {
...
export const projectInformationFragment: PromptFragment<ProjectInformationArgs> = {
    id: "project-information",
    priority: 15,
    template: ({ name, description, repository, tags, hasInventory }) => {
        const parts: string[] = [];
        ...
        if (hasInventory) {
            parts.push("\nThis project has an inventory file with detailed structure information.");
        }

        return parts.join("\n");
    },
    validateArgs: validateProjectInformationArgs,
    expectedArgs:
        "{ name: string, description?: string, repository?: string, tags?: string[], hasInventory?: boolean }",
};

// Auto-register project information fragment
fragmentRegistry.register(projectInformationFragment);
