...
export const availableAgentsFragment: PromptFragment<AvailableAgentsArgs> = {
  ...
  template: ({ agents, currentAgentPubkey }) => {
    ...
    // Find current agent to determine if orchestrator
    const currentAgent = currentAgentPubkey
      ? agents.find((agent) => agent.pubkey === currentAgentPubkey)
      : null;
    const isCurrentAgentOrchestrator = currentAgent?.isOrchestrator || false;
    ...
    const guidance = isCurrentAgentOrchestrator
      ? `**As Orchestrator**: When tasks fall within a specialist's area of expertise, delegate to them using the handoff tool. Let specialists handle implementation details within their domain.`
      : "**As a Specialist**: Focus on your area of expertise. If you receive feedback or tasks that fall outside your specialization, defer to other specialists or the orchestrator agent rather than attempting to handle them yourself.";
