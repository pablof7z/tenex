...
export class PhasePromptBuilder {
  static chatPhase(conversation: ConversationState): string {
    const builder = new PromptBuilder();

    return builder
      .add("base-context", {
        content: "You are the project assistant helping the user clarify their requirements.",
      })
...
4. Determine when we have enough information to create a plan`,
      })
      .add("next-action", {
        availableActions: [
  ...
  static planPhase(conversation: ConversationState, requirements: string): string {
    const builder = new PromptBuilder();
...
6. Potential challenges and mitigations`,
  ...
  static executePhase(conversation: ConversationState, plan: string): string {
    const builder = new PromptBuilder();
...
5. Commit changes with clear messages`,
  ...
  static reviewPhase(conversation: ConversationState, agents: AgentSummary[]): string {
    const builder = new PromptBuilder();
