...
export const findAgentTool: ToolDefinition = {
  ...
  execute: async (params: ToolParameters, context?: ToolContext): Promise<ToolResult> => {
    ...
    // All agents can search for other agents to enable collaboration
    if (!context?.agent) {
      return {
        success: false,
        output: "Agent context not available",
    ...
    try {
      ...
      for (const event of events) {
        ...
        if (agentInfo) {
          ...
          if (
            agentInfo.score > 0 ||
            (!findParams.capabilities && !findParams.specialization && !findParams.keywords)
          ) {
