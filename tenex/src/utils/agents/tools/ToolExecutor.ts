...
export class ToolExecutor {
    ...
    async executeTool(toolCall: ToolCall, context?: ToolContext): Promise<ToolResponse> {
        ...
        let _actualToolName = toolCall.name;
        ...
        if (!tool) {
            ...
            for (const prefix of commonPrefixes) {
                if (toolCall.name.startsWith(prefix)) {
                    ...
                    if (cleanTool) {
                        tool = cleanTool;
                        _actualToolName = cleanName;
                        console.warn(
                            `Tool called with unexpected prefix '${prefix}'. Using '${cleanName}' instead.`
                        );
                        break;
                    }
                }
        ...
        if (!tool) {
            const availableTools = this.registry
                .getAllTools()
                .map((t) => t.name)
                .join(", ");
            return {
