...
export const shellTool: ToolDefinition = {
  name: "shell",
  description: "Execute shell commands with real-time output streaming via ephemeral Nostr events",
  parameters: [
    {
      name: "command",
      type: "string",
  ...
  execute: async (params, context?: ToolContext) => {
    try {
      ...
      const { ndk, rootEventId, projectEvent } = context;
      ...
      if (rootEventId) {
        startEvent.tags.push(["e", rootEventId]);
      }
      ...
      return new Promise((resolve) => {
        ...
        child.stdout.on("data", async (data: Buffer) => {
          ...
          if (rootEventId) {
            stdoutEvent.tags.push(["e", rootEventId]);
          }
        ...
        child.stderr.on("data", async (data: Buffer) => {
          ...
          if (rootEventId) {
            stderrEvent.tags.push(["e", rootEventId]);
          }
        ...
        child.on("close", async (code: number | null) => {
          ...
          if (rootEventId) {
            completeEvent.tags.push(["e", rootEventId]);
          }
        ...
        child.on("error", async (error: Error) => {
          ...
          if (rootEventId) {
            errorEvent.tags.push(["e", rootEventId]);
          }
