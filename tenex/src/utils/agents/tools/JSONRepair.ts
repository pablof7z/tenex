...
const jsonRepairLogger = logger.forModule("agent");
...
export class JSONRepair {
        ...
        try {
            ...
            for (let attempt = 0; attempt < maxRetries; attempt++) {
                try {
                    ...
                    repairedContent = JSONRepair.applyRepairStrategies(
                        repairedContent,
                    ...
                    jsonRepairLogger.info(
                        `JSON successfully repaired after ${attempt + 1} attempts`
                    );
                    return result;
                } catch (repairError) {
     ...
     */
    private static applyRepairStrategies(
        content: string,
        error: Error,
        attempts: string[]
    ): string {
        ...
        const extracted = JSONRepair.extractJSONFromMarkdown(repaired);
        ...
        if (JSONRepair.hasMalformedClosures(repaired)) {
            attempts.push("Fixing malformed object/array closures");
            repaired = JSONRepair.fixMalformedClosures(repaired);
        }

        // Strategy 2: Fix unterminated strings
        if (error.message.includes("Unterminated string")) {
            attempts.push("Applying unterminated string fix");
            repaired = JSONRepair.fixUnterminatedStrings(repaired);
        }

        // Strategy 3: Fix trailing commas
        if (error.message.includes("trailing comma") || JSONRepair.hasTrailingComma(repaired)) {
            attempts.push("Removing trailing commas");
            repaired = JSONRepair.removeTrailingCommas(repaired);
        }
        ...
        if (error.message.includes("Expected property name")) {
            ...
            repaired = JSONRepair.fixUnquotedKeys(repaired);
        }

        // Strategy 5: Fix incomplete JSON structure
        if (JSONRepair.isIncompleteJSON(repaired)) {
            attempts.push("Fixing incomplete JSON structure");
            repaired = JSONRepair.completeJSONStructure(repaired);
        }

        // Strategy 6: Remove comments
        if (JSONRepair.hasComments(repaired)) {
            attempts.push("Removing comments");
            repaired = JSONRepair.removeComments(repaired);
        }

        // Strategy 7: Fix single quotes
        if (JSONRepair.hasSingleQuotes(repaired)) {
            attempts.push("Converting single quotes to double quotes");
            repaired = JSONRepair.fixSingleQuotes(repaired);
        }
    ...
    private static fixUnterminatedStrings(content: string): string {
        ...
        for (let i = 0; i < content.length; i++) {
            ...
            const _nextChar = i < content.length - 1 ? content[i + 1] : "";
