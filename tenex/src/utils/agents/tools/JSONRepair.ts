 ...
 */
export function parseJSON(content: string, options: JSONRepairOptions = {}): unknown {
  ...
  try {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        repairedContent = applyRepairStrategies(
          repairedContent,
 ...
 */
function applyRepairStrategies(content: string, error: Error, attempts: string[]): string {
  let repaired = content;

  // Strategy 0: Extract JSON from markdown code blocks first
  const extracted = extractJSONFromMarkdown(repaired);
  ...
  if (hasMalformedClosures(repaired)) {
    attempts.push("Fixing malformed object/array closures");
    repaired = fixMalformedClosures(repaired);
  }
  ...
  if (error.message.includes("trailing comma") || hasTrailingComma(repaired)) {
 ...
 */
function fixUnquotedKeys(content: string): string {
 ...
 */
function completeJSONStructure(content: string): string {
  let result = content;
  let openBraces = 0;
  let openBrackets = 0;
  let inString = false;
 ...
 */
function hasTrailingComma(content: string): boolean {
 ...
 */
function hasSingleQuotes(content: string): boolean {
 ...
 */
function hasMalformedClosures(content: string): boolean {
 ...
 */
function fixMalformedClosures(content: string): string {
