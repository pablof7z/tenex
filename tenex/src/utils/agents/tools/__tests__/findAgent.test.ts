...
describe("findAgentTool", () => {
    ...
    beforeEach(() => {
        mockContext = {
            agent: {
                ...
                getConfig: () => ({
                    name: "orchestrator",
                    role: "Team orchestrator and coordinator",
                    capabilities: ["orchestration", "team-building"],
                }),
            } as unknown as Agent,
            ndk: {
    ...
    describe("execute", () => {
        test("should reject agents without orchestration capability", async () => {
            ...
            const result = await findAgentTool.execute({}, nonOrchestratorContext);

            expect(result.success).toBe(false);
            expect(result.error).toContain("Only agents with orchestration capability");
            expect(result.renderInChat.data.message).toContain(
                "Only agents with orchestration capability"
            );
        });

        test("should allow agents with orchestrator role", async () => {
            const orchestratorRoleContext = {
                ...mockContext,
                agent: {
                    getName: () => "lead",
                    getConfig: () => ({
                        name: "lead",
                        role: "Lead orchestrator for the team",
            ...
            const mockEvents = new Set([
                createMockNDKAgent("test-agent", "Test Agent", "Testing", "Tests things"),
            ]);

            orchestratorRoleContext.ndk.fetchEvents = mock(() => Promise.resolve(mockEvents));

            const result = await findAgentTool.execute({}, orchestratorRoleContext);
