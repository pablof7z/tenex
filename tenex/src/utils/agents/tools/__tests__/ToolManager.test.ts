...
describe("ToolManager", () => {
    ...
    const _anotherMockTool: ToolDefinition = {
        name: "another_tool",
    ...
    describe("Global Tool Registration", () => {
        it("should register a tool globally and update all agents", () => {
            ...
            expect(agent1Registry?.getAllTools().map((t) => t.name)).toContain("test_tool");
            expect(agent2Registry?.getAllTools().map((t) => t.name)).toContain("test_tool");
        });
        ...
        it("should include global tools in new agent registries", () => {
            ...
            expect(agentRegistry.getAllTools().map((t) => t.name)).toContain("test_tool");
        });
    });
    ...
    describe("Global Tool Unregistration", () => {
        it("should unregister a tool globally from all agents", () => {
            ...
            expect(agent1Registry?.getAllTools().map((t) => t.name)).not.toContain("test_tool");
            expect(agent2Registry?.getAllTools().map((t) => t.name)).not.toContain("test_tool");
        });
    });
