...
// Clean common prefixes that LLMs sometimes add to tool names
function cleanToolName(toolName: string): string {
    const commonPrefixes = ['default_api.', 'api.', 'tools.'];
    
    for (const prefix of commonPrefixes) {
        if (toolName.startsWith(prefix)) {
            const cleaned = toolName.slice(prefix.length);
            console.warn(`Tool call parsed with unexpected prefix '${prefix}'. Cleaned to '${cleaned}'.`);
            return cleaned;
        }
    }
        ...
        try {
            ...
            if (toolData.tool && typeof toolData.tool === "string") {
                ...
                toolCalls.push({
                    ...
                    name: cleanToolName(toolData.tool),
                    arguments: args,
                });
            }
        } catch (_e) {
