 ...
 * Manages event processing, team formation, and agent responses
 */
export class AgentEventHandler {
    ...
    async handleChatEvent(
        event: NDKEvent,
        ndk: NDK,
        _agentName = "code", // No longer using "default"
        llmName?: string,
        ...
        try {
            ...
            logger.debug(` agentsToRespond after determination: ${agentsToRespond.length} agents`);
    ...
    async handleTaskEvent(
        ...
        try {
            ...
            await this.addTaskToAllAgentConversations(event, taskId, title, taskContent);
    ...
    private async addEventToAllAgentConversations(
        ...
        for (const [name, agent] of this.agents) {
            ...
            const context = await this.buildSystemPromptContext(name, false);
            context.additionalRules = await this.getCombinedRulesForAgent(name);
     ...
     */
    private async getCombinedRulesForAgent(agentName: string): Promise<string | undefined> {
    ...
    /**
     * Determine which agents should respond to an event using orchestration
     */
    private async determineRespondingAgents(
        ...
        // Anti-chatter: If event is from an agent and no p-tags, no one responds
        ...
        // First priority: p-tagged agents
        if (mentionedPubkeys.length > 0) {
            ...
            for (const pubkey of mentionedPubkeys) {
                ...
                if (mentionedAgent && mentionedAgent.getPubkey() !== event.author.pubkey) {
                    ...
                    const _eventType = isTaskEvent ? "task" : "conversation";
                    logger.info(
            ...
            // If we have p-tags, use only those agents
            return agentsToRespond;
        }

        // Second priority: Use existing team if available
        ...
        // Third priority: Use orchestration for new conversations (only if not from agent)
        if (!isFromAgent && this.orchestrationCoordinator) {
            try {
                logger.info("No existing team or p-tags, using orchestration to form team");
                ...
                if (orchestrationResult.team) {
                    logger.info(
                        `Orchestration formed team '${orchestrationResult.team.id}' with members: ${orchestrationResult.team.members.join(", ")}`
                    );

                    // Get agents for the team members
                    for (const memberName of orchestrationResult.team.members) {
                        const agent = this.agents.get(memberName);
                        if (agent) {
                            agentsToRespond.push(agent);
                        }
                ...
                // Let error propagate - no fallback
                throw error;
            }
        }
    ...
    private async processAgentResponses(
        ...
        for (const agent of agents) {
            try {
                ...
                const context = await this.buildSystemPromptContext(agent.getName(), false);
                context.additionalRules = await this.getCombinedRulesForAgent(agent.getName());
                const conversation = await agent.getOrCreateConversationWithContext(
                    conversationId,
                    context
                );
    ...
    private async publishTypingIndicator(
        ...
        try {
            ...
            // Debug logging
            logger.debug(`Project event type: ${typeof this.projectInfo.projectEvent}`);
            logger.debug(
                `Project event constructor: ${this.projectInfo.projectEvent.constructor.name}`
            );
            logger.debug(`Project event id: ${this.projectInfo.projectEvent.id}`);
            logger.debug(`Project event kind: ${this.projectInfo.projectEvent.kind}`);

            try {
                // Try alternative approach - manually add the 'a' tag
                const dTag = this.projectInfo.projectEvent.tagValue("d");
                if (dTag) {
                    typingEvent.tags.push([
                        "a",
                ...
                logger.error(`Error tagging typing event: ${tagError}`);
                if (this.projectInfo.projectEvent) {
                    logger.error(
        ...
        try {
            ...
            if (usage) {
                if (usage.prompt_tokens) {
                    responseEvent.tags.push(["llm-input-tokens", String(usage.prompt_tokens)]);
                }
                if (usage.completion_tokens) {
                    responseEvent.tags.push(["llm-output-tokens", String(usage.completion_tokens)]);
                }
                if (usage.total_tokens) {
                    responseEvent.tags.push(["llm-total-tokens", String(usage.total_tokens)]);
                }
            ...
            if (response.confidence !== undefined) {
                responseEvent.tags.push(["llm-confidence", String(response.confidence)]);
            }
     ...
     */
    private logEventError(eventType: string, event: NDKEvent, error: unknown): void {
