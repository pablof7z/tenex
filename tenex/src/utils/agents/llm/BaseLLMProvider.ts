...
export abstract class BaseLLMProvider implements LLMProvider {
    ...
    async generateResponse(
        ...
        return traceFunction(
            `llm.${this.providerName.toLowerCase()}.generateResponse`,
            async (span) => {
                ...
                span?.setAttributes({
                    ...
                    "llm.model": model,
                    "llm.agent": context?.agentName || "unknown",
                    "llm.messages_count": messages.length,
                    "llm.has_tools": Boolean(tools && tools.length > 0),
                    "llm.tools_count": tools ? tools.length : 0,
                });
                ...
                this.logRequest(messages, context, config);
                ...
                try {
                    ...
                    if (!response.ok) {
                        ...
                        span?.setAttributes({
                            "llm.error": true,
                            "llm.error_code": response.status,
                            "llm.error_message": error,
                        });
                        throw new Error(
                    ...
                    // Log response summary and add telemetry
                    this.logResponse(data, model);
                    ...
                    span?.setAttributes({
                        "llm.error": true,
                        "llm.error_message": error instanceof Error ? error.message : String(error),
                    });
                    logger.error(`${this.providerName} provider error: ${error}`);
                    throw error;
                }
            },
            {
                "operation.type": "llm_request",
                "service.name": "tenex-agent",
            }
        );
    }
    ...
    protected logResponse(data: any, model?: string): void {
        const usage = this.extractUsage(data);
    ...
    protected formatToolCallsAsText(toolCalls: any[]): string {
        let content = "";
    ...
    protected abstract extractToolCallData(toolCall: any): { name: string; arguments: any } | null;
}
