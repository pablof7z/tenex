...
export class NostrPublisher implements INostrPublisher {
  ...
  async publishResponse(
    ...
    try {
      ...
      if (response.metadata) {
        ...
        if (metadata.usage) {
          responseEvent.tags.push(["llm-prompt-tokens", metadata.usage.promptTokens.toString()]);
          responseEvent.tags.push([
            "llm-completion-tokens",
            metadata.usage.completionTokens.toString(),
          ]);
          responseEvent.tags.push(["llm-total-tokens", metadata.usage.totalTokens.toString()]);
          ...
          if (metadata.usage.costUsd) {
            responseEvent.tags.push(["llm-cost-usd", metadata.usage.costUsd.toString()]);
          }
        }
      ...
      try {
        ...
        nostrLogger.info(`${agentPrefix}Published response ${responseEvent.id || "unknown-id"}`);
      }
    } catch (error) {
      const agentPrefix = agentName ? `[${agentName}] ` : "";
  ...
  async publishTypingIndicator(
    ...
    try {
      ...
      nostrLogger.warning("Failed to publish typing indicator:", "normal", error);
    }
  }
}
