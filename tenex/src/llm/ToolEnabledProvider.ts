...
export class ToolEnabledProvider implements LLMProvider {
  ...
  async generateResponse(
    ...
    const hasNativeToolCalls = responseWithTools.tool_calls && responseWithTools.tool_calls.length > 0;
    ...
    for (const toolResponse of toolResponses) {
      if (toolResponse.renderInChat) {
        ...
        logger.debug("[ToolEnabledProvider] Found renderInChat data:", this.lastRenderInChat);
        break;
      }
    }
    ...
    if (hasNativeToolCalls) {
      ...
      enhancedMessages.push({
        ...
        tool_calls: responseWithTools.tool_calls,
      });
    ...
    for (const message of finalMessages) {
      ...
      if (message.role !== "system" && (!message.content || message.content.trim().length < 2)) {
        continue;
      }
