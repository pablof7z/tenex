...
export function createLLMProvider(
  ...
  try {
    ...
    logger.debug(`Created LLM provider: ${config.provider} ${toolRegistry ? "with tools" : ""}`);
    return provider;
  } catch (error) {
...
function createLegacyProvider(
  ...
  switch (config.provider.toLowerCase()) {
    case "anthropic":
      baseProvider =
        config.enableCaching !== false ? new AnthropicProviderWithCache() : new AnthropicProvider();
      break;
    case "openai":
