...
export abstract class BaseLLMProvider implements LLMProvider {
  ...
  protected logRequest(messages: LLMMessage[], context?: LLMContext, config?: LLMConfig): void {
    ...
    if (context) {
      ...
      const otherMessages = messages.filter((msg) => msg.role !== "system" && msg.role !== "user");
      ...
      logger.info(chalk.cyan(`   System message: ${systemMessage?.content?.length || 0} chars`));
    ...
    if (usage) {
      ...
      if (content) {
        ...
        if (totalTokens > 0) {
          ...
          logger.info(chalk.yellow(`   Efficiency: ${charsPerToken.toFixed(2)} chars/token`));
        }
      }
  ...
  protected abstract extractToolCallData(toolCall: unknown): { name: string; arguments: Record<string, unknown> } | null;
}
