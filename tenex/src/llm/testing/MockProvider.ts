...
export class MockLLMProvider extends BaseLLMProvider {
  ...
  protected async makeRequest(
    ...
    if (mockResponse.shouldFail) {
      return new Response(JSON.stringify({ error: mockResponse.failureReason || "Mock failure" }), {
        status: 500,
      });
    }
  ...
  private parseAnthropicResponse(data: Record<string, unknown>): LLMResponse {
    ...
    return {
      ...
      usage: data.usage
        ? {
            prompt_tokens: (data.usage as any).input_tokens,
            completion_tokens: (data.usage as any).output_tokens,
            total_tokens: (data.usage as any).input_tokens + (data.usage as any).output_tokens,
          }
        : undefined,
    };
  }
  ...
  private parseOpenAIResponse(data: Record<string, unknown>): LLMResponse {
    ...
    return {
      content: (choice as any)?.message?.content || "",
      model: data.model as any,
  ...
  private parseOllamaResponse(data: Record<string, unknown>): LLMResponse {
    return {
      ...
      model: data.model as any,
      usage: {
        prompt_tokens: (data.prompt_eval_count as any) || 0,
        completion_tokens: (data.eval_count as any) || 0,
        total_tokens: ((data.prompt_eval_count as any) || 0) + ((data.eval_count as any) || 0),
      },
    };
  }
  ...
  protected extractUsage(
    ...
    if (dataAny.usage) {
      if (dataAny.usage.input_tokens !== undefined && dataAny.usage.output_tokens !== undefined) {
        // Anthropic format
        return {
          prompt_tokens: dataAny.usage.input_tokens,
          completion_tokens: dataAny.usage.output_tokens,
        };
      }
      if (
        dataAny.usage.prompt_tokens !== undefined &&
        dataAny.usage.completion_tokens !== undefined
      ) {
        // OpenAI format
        return {
          prompt_tokens: dataAny.usage.prompt_tokens,
          completion_tokens: dataAny.usage.completion_tokens,
        };
      }
    }
    return null;
