...
export class MockLLMProvider extends BaseLLMProvider {
    ...
    private createMockResponseData(mockResponse: MockResponse, config: LLMConfig): any {
    ...
    private createOllamaFormat(mockResponse: MockResponse): any {
        const toolCalls = mockResponse.toolCalls?.map((toolCall) => ({
            id: toolCall.id || `call_${Math.random().toString(36).substr(2, 9)}`,
            type: "function" as const,
            function: {
    ...
    private parseOllamaResponse(data: any): LLMResponse {
        return {
