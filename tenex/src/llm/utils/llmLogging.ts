...
export function logLLMError(providerName: string, errorMessage: string, errorDetails?: unknown): void {
...
function logRequestSummary(summary: RequestSummary): void {
    ...
    llmLogger.info(
        chalk.cyan(`   System message: ${summary.systemPromptLength} chars`),
        "verbose"
    );
...
function extractUsage(response: unknown): ResponseSummary["usage"] | undefined {
    ...
    if (usage) {
        return {
            ...
            completion_tokens:
                (usage.completion_tokens as number) || (usage.output_tokens as number),
...
function hasToolCalls(response: unknown): boolean {
    ...
    if (Array.isArray(resp.content)) {
        return resp.content.some(
            (c: unknown) => (c as Record<string, unknown>).type === "tool_use"
        );
    }
