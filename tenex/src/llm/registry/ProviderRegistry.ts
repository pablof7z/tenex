...
export class ProviderRegistry {
  ...
  static create(config: LLMConfig): LLMProvider {
    const cacheKey = ProviderRegistry.getCacheKey(config);

    // Return cached instance if available
    if (ProviderRegistry.instances.has(cacheKey)) {
      const cachedInstance = ProviderRegistry.instances.get(cacheKey);
      if (!cachedInstance) {
        throw new Error(`Cached provider instance not found for key: ${cacheKey}`);
      }
    ...
    const provider = providerInfo.factory.create();
  ...
  static findBestProvider(
    ...
    return candidates.length > 0 ? candidates[0]?.[0] || null : null;
  }
  ...
  static validateProvider(
    ...
    if (!providerInfo) {
      throw new Error(`Provider info not found for provider: ${provider}`);
    }
