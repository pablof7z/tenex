...
import { formatMarkdown, colorizeJSON } from "../src/utils/formatting";
...
function getContentSummary(content: string, maxLength = 60): string {
...
function getMultiLinePreview(content: string, maxLines = 10): string {
...
function buildTree(entries: LLMCallLogEntry[]): TreeNode[] {
    return entries.map((entry, entryIndex) => {
        ...
        const requestPreview = lastUserMsg
            ? getContentSummary(lastUserMsg.content, 80)
            : "No user message";
        ...
        if (entry.response) {
            const responseNode: TreeNode = {
                ...
                entryIndex,
                metadata: {
                    preview: entry.response.content
                        ? getContentSummary(entry.response.content, 60)
                        : null,
                },
            };
...
function displayTree(state: NavigationState) {
    ...
    console.log(
        chalk.gray(
            `${state.entries.length} entries | Use arrows to navigate, Enter to expand/collapse, / to search`
        )
    );
    ...
    if (currentVisibleIndex >= 0) {
        ...
        for (
            let i = state.viewportStart;
            i < visibleNodes.length && viewportLines < availableHeight;
            i++
        ) {
        ...
        // Scroll down if current node is below viewport
            ...
            for (
                ...
                i >= 0 && testLines + nodeLineCount[i] <= availableHeight;
                i--
            ) {
    ...
    for (
        let i = state.viewportStart;
        i < visibleNodes.length && linesShown < availableHeight;
        i++
    ) {
        ...
        // Format the line
        const line = indent + expandIndicator + node.label;
        ...
        if (node.type === "entry" && node.expanded && node.metadata) {
            // Show metadata
            console.log(
                `${indent}    ${chalk.gray(`Duration: ${node.metadata.duration} | Agent: ${node.metadata.agent || "N/A"}`)}`
            );
            linesShown++;
        }
            ...
            if (node.label.includes("REQUEST") && node.metadata.lastMessage) {
                ...
                for (const word of lines) {
                    if (currentLine.length + word.length + 1 > 70 && lineCount < 3) {
                        console.log(
                            `${indent}    ${lineCount === 0 ? chalk.gray("Preview: ") : "         "}${chalk.magenta(currentLine)}`
                        );
                        currentLine = word;
                        lineCount++;
                        linesShown++;
        ...
        if (node.type === "section" && node.expanded && node.label.includes("RESPONSE")) {
            ...
            if (entry.response?.content) {
                ...
                if (linesShown < availableHeight - 2 && entry.response.usage) {
                    ...
                    console.log(
                        `${indent}  ${chalk.gray(`Tokens: ${entry.response.usage.totalTokens || 0} | Cost: $${entry.response.usage.cost?.toFixed(4) || "0.00"}`)}`
                    );
                    linesShown += 3;
                }
            }
    ...
    console.log(chalk.gray("═".repeat(80)));
    console.log(
        chalk.gray("↑/↓ Navigate | ←/→ Collapse/Expand | Enter Toggle | / Search | q Quit")
    );
}
    ...
    try {
        ...
        process.stdin.on("keypress", (str, key) => {
            ...
            if (key.name === "up") {
                ...
                if (
                    currentNode.content &&
                    (currentNode.type === "message" || currentNode.type === "field")
                ) {
