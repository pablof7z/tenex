...
export function RecentTasksList({ project }: RecentTasksListProps) {
    ...
    const tasks = useMemo(() => {
        return (rawTasks || [])
            .map((event) => NDKTask.from(event as NDKEvent))
    ...
    const handleStartTask = async (task: NDKTask) => {
        ...
        const taskTitle =
            task.tags.find((t) => t[0] === "title")?.[1] ||
            task.content.substring(0, 70) + (task.content.length > 70 ? "..." : "") ||
            "Untitled Task";
                        ...
                        >
                            <div className="flex justify-between items-center">
                                <Link href={`/project/${project.slug}/${taskUrlId}`} passHref legacyBehavior>
                                    <a
                                        className="text-xs text-muted-foreground font-medium hover:underline flex-grow mr-2 truncate"
                                        title={title}
                                    >
                                        {title}
                                    </a>
