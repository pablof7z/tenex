...
export async function POST(request: NextRequest) {
  ...
  if (!isValidCmd(cmd)) {
    return NextResponse.json({ error: 'Invalid or missing "cmd" parameter. Must be 1-20 letters.' }, { status: 400 });
  }
  ...
  try {
    ...
    return NextResponse.json({ error: `Script "${cmd}" not found or not executable at ${scriptsPath}.` }, { status: 404 });
  }
  ...
  try {
    ...
    try { // Inner try block for script execution and waiting
    // 5. Execute the script, piping stdin
    const scriptProcess = spawn(scriptPath, [], { // No file path argument
