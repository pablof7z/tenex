    ...
    for (const dirent of projectDirs) {
      ...
      if (dirent.isDirectory()) {
        ...
        try {
          ...
          if (error instanceof Error && 'code' in error && error.code !== 'ENOENT' && !(error instanceof SyntaxError)) {
            console.error(`Error processing project ${dirent.name}:`, error);
            // Decide if you want to return an error or just log and skip
          } else if (error instanceof Error && 'code' in error && error.code !== 'ENOENT') { // Add type guard here too
