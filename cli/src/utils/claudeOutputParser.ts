	...
	private handleSystemMessage(data: ClaudeMessage): void {
		if (data.subtype === "init") {
			...
			if (data.mcp_servers && Array.isArray(data.mcp_servers)) {
				const connectedServers = data.mcp_servers.filter(
		...
		for (const item of content) {
			if (item.type === "tool_result") {
				...
				if (contentLines.length <= maxLines) {
					...
					console.log(
						chalk.gray(`... (${contentLines.length - maxLines} more lines)`),
					);
				}
			}
		}
	...
	flush(): void {
		if (this.buffer.trim()) {
			try {
				...
				console.error(
					chalk.red(`Failed to parse remaining buffer: ${this.buffer}`),
				);
			}
			this.buffer = "";
		}
