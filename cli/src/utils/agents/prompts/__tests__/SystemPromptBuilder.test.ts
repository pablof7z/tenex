...
describe("SystemPromptBuilder", () => {
	...
	describe("basic functionality", () => {
		...
		it("should use predefined system prompt if available", () => {
			...
			expect(prompt).toBe("This is a predefined system prompt");
	...
	describe("section ordering", () => {
		it("should order sections by priority", () => {
			...
			const staticIndex = sections.findIndex((s) =>
				s.includes("TENEX System Instructions"),
			);
			const identityIndex = sections.findIndex((s) =>
				s.includes("Your Identity"),
			);
			expect(staticIndex).toBeLessThan(identityIndex);

			// Project rules (60) should come after team information (70)
			const teamIndex = sections.findIndex((s) =>
				s.includes("Available Agents"),
			);
	...
	describe("preview functionality", () => {
		it("should provide section preview with metadata", () => {
			...
			const staticSection = preview.sections.find(
				(s) => s.id === "static-instructions",
			);
