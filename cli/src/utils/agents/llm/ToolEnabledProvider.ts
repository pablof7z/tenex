...
export class ToolEnabledProvider implements LLMProvider {
	...
	async generateResponse(
		...
		logger.debug(
			"[ToolEnabledProvider] Initial response content:",
			response.content,
		);
		...
		logger.debug(
			`[ToolEnabledProvider] Parsed ${toolCalls.length} tool calls:`,
			toolCalls,
		);
		...
		if (toolCalls.length === 0) {
			...
			logger.debug(
				"[ToolEnabledProvider] No tool calls found, returning response as-is",
			);
			return response;
		}
		...
		const toolResponses = await this.executor.executeTools(
			toolCalls,
			toolContext,
		);
		logger.debug(
		...
		logger.debug(
			"[ToolEnabledProvider] Getting final response after tool execution",
		);
		logger.debug(
		...
		logger.debug(
			"[ToolEnabledProvider] Final response content:",
		...
		logger.debug(
			"[ToolEnabledProvider] Final cleaned content:",
			finalCleanedContent,
		);
		...
		if (response.usage && finalResponse.usage) {
			finalResponse.usage = {
				prompt_tokens:
					response.usage.prompt_tokens + finalResponse.usage.prompt_tokens,
				completion_tokens:
					response.usage.completion_tokens +
					finalResponse.usage.completion_tokens,
				total_tokens:
					response.usage.total_tokens + finalResponse.usage.total_tokens,
