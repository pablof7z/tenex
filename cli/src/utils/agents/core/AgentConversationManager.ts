...
export class AgentConversationManager {
	...
	async createConversation(
		...
		const systemPrompt = this.agentCore.getSystemPrompt(
			additionalRules,
			environmentContext,
		);
	...
	async getOrCreateConversation(
		...
		if (!conversation && this.storage) {
			...
			const savedContext = await this.storage.loadConversation(
				conversationId,
				this.agentCore.getName(),
			);
			if (savedContext && savedContext.agentName === this.agentCore.getName()) {
				// Only load if this conversation belongs to this agent
				conversation = Conversation.fromJSON(savedContext);
				this.conversations.set(conversationId, conversation);
				this.agentCore
					.getLogger()
					.info(`Loaded conversation ${conversationId} from storage`);
			} else if (savedContext) {
				this.agentCore
					.getLogger()
					.debug(
						`Found conversation ${conversationId} but it belongs to agent '${savedContext.agentName}', not '${this.agentCore.getName()}'`,
					);
			}
		}
