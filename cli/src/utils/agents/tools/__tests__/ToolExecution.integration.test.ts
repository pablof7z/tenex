...
<tool_use>
{
  ...
  "arguments": {}
			...
			(mockProvider.generateResponse as any).mockResolvedValue(llmResponse);
			...
			expect(result.content).toContain(
				"SPEC.md contains project documentation",
			);
...
<tool_use>
{
  ...
  "arguments": {
    "spec_name": "ARCHITECTURE"
			...
			expect(timeTool.execute).toHaveBeenCalledWith(
				{ format: "unix" },
				expect.any(Object),
			);
			...
			expect(result.content).toContain(
				"Current time (unix): 2024-01-13T10:30:00Z",
			);
			expect(result.content).toContain(
				"ARCHITECTURE.md: Project documentation loaded",
			);
		});
  ...
  "arguments": {}
	...
	describe("Native function calling execution (like Claude/GPT)", () => {
		test("should handle responses without text-based tool calls", async () => {
			...
			(mockProvider.generateResponse as any).mockResolvedValue(normalResponse);
	...
	describe("Tool registry system prompts", () => {
		test("should add tool instructions to system prompt when tools are available", async () => {
			...
			const calledMessages = (mockProvider.generateResponse as any).mock
				.calls[0][0];
			const systemMessage = calledMessages.find(
				(m: any) => m.role === "system",
			);
