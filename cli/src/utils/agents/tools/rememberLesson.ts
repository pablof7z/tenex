		...
		try {
			...
			if (!context.agent || !context.agentEventId) {
				return {
					...
					output: "",
					error:
						"Missing agent or agentEventId in context for remember_lesson tool",
				};
			}
			...
			logger.info(
				`Agent '${context.agentName}' recorded lesson: ${params.title}`,
			);
			...
			return {
				...
				output: "",
				error:
					error instanceof Error ? error.message : "Failed to record lesson",
			};
		}
	},
};
