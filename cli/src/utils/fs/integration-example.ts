...
export class ConfigurationManager {
    ...
    async update<T extends object>(updater: (config: T) => T | Promise<T>): Promise<T> {
...
export class ProjectManager {
    ...
    async createProject(name: string, template?: string): Promise<string> {
        ...
        await this.fs.writeJSON(this.fs.join(projectPath, ".tenex/project.json"), projectConfig);
    ...
    async listProjects(): Promise<Array<{ name: string; path: string; config: unknown }>> {
...
export async function testExample() {
    ...
    const configManager = new ConfigurationManager("~/.tenex/config.json", mockFs);
...
const _configManager = services.createConfigManager("~/.tenex/config.json");

// Usage in tests
const mockServices = new CliServices(new MockFileSystem());
const _testConfigManager = mockServices.createConfigManager("/test/config.json");
