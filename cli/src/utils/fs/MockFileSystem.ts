...
export class MockFileSystem implements IFileSystem {
	...
	readFileSync(
		...
		if (!file) {
			throw new Error(
				`ENOENT: no such file or directory, open '${expandedPath}'`,
			);
		}
	...
	writeFileSync(
		...
		const content =
			typeof data === "string" && encoding ? Buffer.from(data, encoding) : data;
	...
	existsSync(filePath: string): boolean {
		const expandedPath = this.expandHome(filePath);
		return (
			this.data.files.has(expandedPath) ||
			this.data.directories.has(expandedPath)
		);
	}
	...
	mkdirSync(dirPath: string, options?: { recursive?: boolean }): void {
		...
		if (options?.recursive) {
			...
			if (!this.data.directories.has(parent)) {
				throw new Error(
					`ENOENT: no such file or directory, mkdir '${expandedPath}'`,
				);
			}
	...
	readdirSync(dirPath: string): string[] {
		...
		if (!this.data.directories.has(expandedPath)) {
			throw new Error(
				`ENOENT: no such file or directory, scandir '${expandedPath}'`,
			);
		}
	...
	statSync(filePath: string): Stats {
		...
		throw new Error(
			`ENOENT: no such file or directory, stat '${expandedPath}'`,
		);
	}
	...
	unlinkSync(filePath: string): void {
		...
		if (!this.data.files.has(expandedPath)) {
			throw new Error(
				`ENOENT: no such file or directory, unlink '${expandedPath}'`,
			);
		}
	...
	rmdirSync(dirPath: string, options?: { recursive?: boolean }): void {
		...
		if (!this.data.directories.has(expandedPath)) {
			throw new Error(
				`ENOENT: no such file or directory, rmdir '${expandedPath}'`,
			);
		}
		...
		if (options?.recursive) {
			...
			const hasContents =
				Array.from(this.data.files.keys()).some(
					(f) => path.dirname(f) === expandedPath,
				) ||
			...
			if (hasContents) {
				throw new Error(
					`ENOTEMPTY: directory not empty, rmdir '${expandedPath}'`,
				);
			}
	...
	async readFile(
		...
		return this.readFileSync(filePath, encodingOrOptions as any);
	}
