...
import { INSTRUCTION_KIND, type GetRuleOptions } from "./types";
...
export const getCommand = new Command("get")
    ...
    .action(async (searchTerm: string, options: GetRuleOptions) => {
        try {
            ...
            const { selectedIndices } = await inquirer.prompt([
                {
                    ...
                    choices,
                    validate: (input) => input.length > 0 || "Please select at least one rule",
                },
            ]);
            ...
            logger.error(`Failed to search rules: ${error instanceof Error ? error.message : String(error)}`);
            process.exit(1);
        }
    });