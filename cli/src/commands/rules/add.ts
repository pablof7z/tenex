...
import { INSTRUCTION_KIND, type AddRuleOptions } from "./types";
    ...
    .action(async (eventIdArg: string, options: AddRuleOptions) => {
        try {
            ...
            if (!hasProjectConfig(projectPath)) {
                logger.error("No .tenex directory found. Please run from a TENEX project directory.");
                process.exit(1);
            }
            ...
            if (event.kind !== INSTRUCTION_KIND) {
                logger.error(`Event is not an instruction (kind ${event.kind}, expected ${INSTRUCTION_KIND})`);
                process.exit(1);
            }

            const title = event.tags.find(tag => tag[0] === "title")?.[1] || "Untitled Rule";
            ...
            logger.error(`Failed to add rule: ${error instanceof Error ? error.message : String(error)}`);
            process.exit(1);
        }
    });