        ...
        try {
            ...
            if (options.agent) {
                ...
                if (general.length === 0 && byAgent.size === 0) {
                    ...
                    logger.info("Use 'tenex rules get <search-term>' to add rules from Nostr.");
                    process.exit(0);
                }
            ...
            logger.error(`Failed to list rules: ${error instanceof Error ? error.message : String(error)}`);
            process.exit(1);
        }
    });