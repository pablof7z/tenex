...
- **`GitBranchManager`** (`git/branch.ts`) - Git branch workflow management:
  - `ensureMainBranch()` - Wait for main/master branch
  - `createAndSwitch()` - Create and checkout new branches
  - `mergeToMain()` - Merge branches back to main
...
11. **Parser Pattern** - Structured output processing
...
4. **Quality standards**:
   ...
   - Maintain type safety throughout. No "any" -- always use strong typing and runtime guards when necessary.
