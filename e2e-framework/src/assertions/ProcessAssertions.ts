...
export async function assertProcessOutput(
    ...
    throw new TestError(`Process ${stream} ended without containing "${expected}"`, {
        step: "assertProcessOutput",
    });
}
...
export async function assertProcessExits(
    ...
    return new Promise((resolve, reject) => {
        const timeoutId = setTimeout(() => {
            reject(
                new TestError(`Process did not exit within ${actualTimeout}ms`, {
        ...
        handle.process.on("exit", (code) => {
            ...
            if (code !== expectedCode) {
                reject(
                    new TestError(`Process exited with code ${code}, expected ${expectedCode}`, {
                        step: "assertProcessExits",
                    })
                );
            } else {
...
export function assertProcessRunning(handle: ProcessHandle): void {
    if (handle.process.killed) {
        throw new TestError("Process has been killed", {
            step: "assertProcessRunning",
        });
    }

    if (handle.process.exitCode !== null) {
        throw new TestError(`Process has exited with code ${handle.process.exitCode}`, {
            step: "assertProcessRunning",
        });
    }
}
...
export async function assertProcessOutputPattern(
    ...
    throw new TestError(`Process ${stream} ended without matching pattern ${pattern}`, {
        step: "assertProcessOutputPattern",
    });
}
