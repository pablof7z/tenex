 ...
 * ```
 */
export async function assertFileExists(project: Project, filePath: string): Promise<void> {
    const exists = await project.fileExists(filePath);
    if (!exists) {
        throw new TestError(`Expected file ${filePath} to exist but it does not`, {
            project: project.name,
 ...
 * ```
 */
export async function assertFileDoesNotExist(project: Project, filePath: string): Promise<void> {
    const exists = await project.fileExists(filePath);
    if (exists) {
        throw new TestError(`Expected file ${filePath} not to exist but it does`, {
            project: project.name,
