...
import type { LLMConfig, TenexConfig, ProjectOptions, WaitOptions } from "./types";
...
export class Orchestrator {
    ...
    constructor(private options: OrchestratorOptions = {}) {
        ...
        this.nostrMonitor = new NostrMonitor(options.relays || config.nostr.defaultRelays);
    ...
    private async waitForDaemonReady(): Promise<void> {
        ...
        for await (const line of this.daemonHandle.stdout) {
            ...
            if (
                !ready &&
                (line.includes("TENEX daemon is running") ||
                    line.includes("Monitoring events from") ||
                    line.includes("Starting TENEX daemon") ||
                    line.includes("Whitelisted pubkeys"))
            ) {
                ready = true;
