...
describe("ProcessAssertions", () => {
    ...
    describe("assertProcessOutput", () => {
        ...
        test("should throw on timeout", async () => {
            ...
            await expect(assertProcessOutput(mockHandle, "test", { timeout: 50 })).rejects.toThrow(
                'Process stdout did not contain "test" within 50ms'
            );
        });
        ...
        test("should throw when stream ends without match", async () => {
            ...
            await expect(assertProcessOutput(mockHandle, "baz")).rejects.toThrow(
                'Process stdout ended without containing "baz"'
            );
        });
    });
    ...
    describe("assertProcessExits", () => {
        test("should pass when process exits with expected code", async () => {
            ...
            await expect(assertProcessExits(mockHandle, 0)).resolves.toBeUndefined();
        });
        ...
        test("should throw when process exits with wrong code", async () => {
            ...
            await expect(assertProcessExits(mockHandle, 0)).rejects.toThrow(
        ...
        test("should throw on timeout", async () => {
            ...
            await expect(assertProcessExits(mockHandle, 0, 50)).rejects.toThrow(
    ...
    describe("collectProcessOutput", () => {
        ...
        test("should respect maxLines limit", async () => {
            ...
            const result = await collectProcessOutput(mockHandle, {
                maxLines: 3,
                stream: "stdout",
            });
    ...
    describe("assertProcessRunning", () => {
        ...
        test("should throw when process is killed", () => {
            ...
            expect(() => assertProcessRunning(mockHandle)).toThrow("Process has been killed");
        });

        test("should throw when process has exited", () => {
            mockProcess.exitCode = 0;

            expect(() => assertProcessRunning(mockHandle)).toThrow(
    ...
    describe("assertProcessOutputPattern", () => {
        ...
        test("should find pattern in stderr", async () => {
            ...
            const result = await assertProcessOutputPattern(mockHandle, /Error:.*database/, {
                stream: "stderr",
            });
        ...
        test("should throw when pattern not found", async () => {
            ...
            await expect(assertProcessOutputPattern(mockHandle, /test/)).rejects.toThrow(
                "Process stdout ended without matching pattern /test/"
            );
        });
