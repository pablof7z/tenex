...
describe("FileAssertions", () => {
    ...
    describe("assertFileContains", () => {
        ...
        test("should throw when file does not contain expected string", async () => {
            ...
            await expect(assertFileContains(mockProject, "test.txt", "Goodbye")).rejects.toThrow(
                'File test.txt does not contain "Goodbye"'
            );
        });
    });
    ...
    describe("assertFileExists", () => {
        test("should pass when file exists", async () => {
            ...
            await expect(assertFileExists(mockProject, "test.txt")).resolves.toBeUndefined();
        });

        test("should throw when file does not exist", async () => {
            mockProject.fileExists = mock(async () => false);

            await expect(assertFileExists(mockProject, "test.txt")).rejects.toThrow(
                "Expected file test.txt to exist but it does not"
            );
        });
    });
    ...
    describe("assertFileDoesNotExist", () => {
        test("should pass when file does not exist", async () => {
            ...
            await expect(assertFileDoesNotExist(mockProject, "test.txt")).resolves.toBeUndefined();
        });

        test("should throw when file exists", async () => {
            mockProject.fileExists = mock(async () => true);

            await expect(assertFileDoesNotExist(mockProject, "test.txt")).rejects.toThrow(
                "Expected file test.txt not to exist but it does"
            );
        });
    });
    ...
    describe("assertFileSize", () => {
        test("should pass when file size is within range", async () => {
            ...
            await expect(assertFileSize(mockProject, "test.txt", 10, 20)).resolves.toBeUndefined();
        });

        test("should throw when file is too small", async () => {
            mockProject.readFile = mock(async () => "Hi"); // 2 bytes

            await expect(assertFileSize(mockProject, "test.txt", 10)).rejects.toThrow(
                "File test.txt is too small. Expected at least 10 bytes but got 2"
            );
        });
        ...
        test("should throw when file is too large", async () => {
            ...
            await expect(assertFileSize(mockProject, "test.txt", 10, 50)).rejects.toThrow(
                "File test.txt is too large. Expected at most 50 bytes but got 100"
            );
        });
        ...
        test("should work with only minSize", async () => {
            ...
            await expect(assertFileSize(mockProject, "test.txt", 10)).resolves.toBeUndefined();
        });
    });
    ...
    describe("assertJsonFile", () => {
        ...
        test("should throw on invalid JSON", async () => {
            ...
            await expect(assertJsonFile(mockProject, "test.json")).rejects.toThrow(
                "File test.json is not valid JSON"
            );
        });
        ...
        test("should run validator function on JSON", async () => {
            ...
            await expect(assertJsonFile(mockProject, "test.json", validator)).resolves.toEqual(
                jsonContent
            );
        });
        ...
        test("should throw when validator fails", async () => {
            ...
            await expect(assertJsonFile(mockProject, "test.json", validator)).rejects.toThrow(
                "JSON validation failed for test.json: Value must be 42"
            );
        });
    });
});
