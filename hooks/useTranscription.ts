...
import { TranscriptionResponse, TranscriptionError } from '@/types/audio';
...
export function useTranscription(options: UseTranscriptionOptions = {}) {
    ...
    const transcribe = useCallback(async (audioBlob: Blob): Promise<TranscriptionResponse> => {
        if (!audioBlob) {
    ...
    const transcribeWithFeedback = useCallback(async (audioBlob: Blob) => {
        try {
