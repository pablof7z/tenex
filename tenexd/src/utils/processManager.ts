...
export class ProcessManager {
    ...
    constructor(private configManager: ConfigManager) {
        // Default tenex command
        this.tenexCommand = "npx tenex";
    ...
    private async initFromConfig() {
        ...
        if (config.tenexCommand) {
            this.tenexCommand = config.tenexCommand;
        }
    }
    ...
    private async initializeLLMsFile(projectPath: string): Promise<void> {
        ...
        try {
            ...
            try {
                ...
                const llmsConfig: Record<string, unknown> = {};
                ...
                logError(
                    `Failed to create llms.json: ${err instanceof Error ? err.message : String(err)}`
                );
            }
        }
    }
    ...
    async startProject(projectId: string, projectPath: string): Promise<void> {
        ...
        try {
            ...
            console.log(
                chalk.gray("Command:   ") + chalk.yellow(`${this.tenexCommand} ${args.join(" ")}`)
            );
            console.log(chalk.blue("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"));
            ...
            logError(
                `[${projectId}] Failed to start project: ${err instanceof Error ? err.message : String(err)}`
            );
        }
    }
    ...
    stopProject(projectId: string): void {
        ...
        try {
            ...
            logError(
                `[${projectId}] Failed to stop project: ${err instanceof Error ? err.message : String(err)}`
            );
        }
    }
    ...
    private cleanup(): void {
        ...
        this.runningProjects.forEach((runningProject, _projectId) => {
            try {
                runningProject.process.kill("SIGTERM");
