...
export class ConfigManager {
    ...
    async removeAIConfiguration(name: string): Promise<void> {
        ...
        config.aiConfigurations = config.aiConfigurations.filter((c) => c.name !== name);

        // Clear default if it was the removed configuration
        if (config.defaultAIConfiguration === name) {
            config.defaultAIConfiguration =
                config.aiConfigurations.length > 0 ? config.aiConfigurations[0].name : undefined;
        }

        this.save();
    }

    async getAIConfiguration(name?: string): Promise<AIConfiguration | undefined> {
        const config = await this.getConfig();
    ...
    private async migrateAIConfig(): Promise<void> {
        ...
        if (this.config.aiConfigurations && this.config.aiConfigurations.length > 0) {
            return;
        }

        // Check if old ai-config.json exists
        const oldAIConfigPath = "./ai-config.json";
        ...
        try {
            ...
            if (oldConfig.configurations && Array.isArray(oldConfig.configurations)) {
                ...
                (this.config as Record<string, unknown>).aiConfigPath = undefined;

                // Save the migrated config
                this.save();

                console.log("✅ Migrated AI configurations from ai-config.json to config.json");
            }
        } catch (err) {
