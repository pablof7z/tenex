...
export async function handleProjectEvent(
	...
	if (!projectTag || !projectTag.startsWith("31933:")) {
	...
	try {
		...
		if (projectInfo.exists) {
			...
			logWarning(
				`[${projectDTag}] Project not found locally at ${projectInfo.path}`,
			);
			...
			try {
				...
				logError(
					`[${projectDTag}] Failed to initialize project: ${err.message}`,
				);
				return;
			}
		}
		...
		logError(`Failed to handle project event: ${err.message}`);
	}
}
...
export async function handleAgentEvent(
	...
	try {
		...
		if (!agentEntry) {
			...
			needsUpdate = true;
			logInfo(
				`[${projectIdentifier}] Updated agent ${agentName} to new format`,
			);
		} else {
			...
			if (!agentEntry.file || agentEntry.file !== `${event.id}.json`) {
				...
				needsUpdate = true;
				logInfo(
					`[${projectIdentifier}] Updated file reference for agent ${agentName}`,
				);
			}
		}
		...
		if (needsUpdate) {
			...
			try {
				...
				logInfo(
					`[${projectIdentifier}] Agent profile prepared for ${agentDisplayName}`,
				);
			} catch (err: any) {
				logWarning(
					`[${projectIdentifier}] Could not prepare agent profile: ${err.message}`,
				);
			}
		}

		logSuccess(
			`[${projectIdentifier}] Saved agent configuration for ${agentName}`,
		);
	} catch (err: any) {
