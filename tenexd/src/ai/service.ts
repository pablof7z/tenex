...
import type { AICompletionOptions, AICompletionResponse, AIModelConfig } from "./types.js";
...
export class AIService {
    ...
    async complete(
        messagesOrOptions: OpenAI.Chat.ChatCompletionMessageParam[] | AICompletionOptions
        ...
        try {
            ...
            if (error instanceof Error) {
                ...
                if (openAIError.status === 404) {
                    throw new Error(`Model ${this.config.model} not found for ${this.config.name}`);
                }
                throw new Error(`AI completion failed: ${error.message}`);
            }
            throw new Error(`AI completion failed: ${String(error)}`);
        }
    }
    ...
    async testConnection(): Promise<boolean> {
        try {
            ...
            console.error(chalk.red(`Failed to test connection for ${this.config.name}:`), error);
            return false;
        }
    }
