...
export class AIConfigManager {
	...
	async addConfiguration(modelConfig: AIModelConfig): Promise<void> {
		...
		const existing = config.configurations.find(
			(c) => c.name === modelConfig.name,
		);
		if (existing) {
			throw new Error(
				`Configuration with name '${modelConfig.name}' already exists`,
			);
		}
	...
	async updateConfiguration(
		...
		if (updates.name && updates.name !== name) {
			const existing = config.configurations.find(
				(c) => c.name === updates.name,
			);
			if (existing) {
				throw new Error(
					`Configuration with name '${updates.name}' already exists`,
				);
			}
		}
	...
	async getService(name?: string): Promise<AIService | null> {
		...
		if (!name) {
			name = config.defaultConfiguration;
		}
