...
export async function setupAIConfiguration(configManager: ConfigManager): Promise<void> {
    ...
    if (configurations.length === 0) {
        console.log(chalk.yellow("AI configuration is required. Let's set one up!\n"));
        await addNewConfiguration(configManager);

        // After first config, ask if they want to add more
        const updatedConfig = await configManager.getConfig();
        if (updatedConfig.aiConfigurations && updatedConfig.aiConfigurations.length > 0) {
        ...
        console.log(chalk.green(`Found ${configurations.length} AI configuration(s):`));
        configurations.forEach((aiConfig, index) => {
            const isDefault = aiConfig.name === config.defaultAIConfiguration;
            console.log(
                chalk.gray(`  ${index + 1}. ${aiConfig.name}${isDefault ? " (default)" : ""}`)
            );
        });
        console.log();

        console.log(chalk.yellow("Would you like to add another configuration? (y/n)"));
        process.stdout.write(chalk.cyan("> "));
...
async function addNewConfiguration(configManager: ConfigManager): Promise<void> {
    console.log(chalk.cyan("\n📝 Adding new AI configuration\n"));

    // Configuration name
    console.log(
        chalk.yellow("Enter a name for this configuration (e.g., 'default', 'fast', 'smart'):")
    );
    process.stdout.write(chalk.cyan("> "));
    ...
    for await (const line of console) {
        apiKey = line.trim();
        if (apiKey) break;
        console.log(chalk.red("API key cannot be empty. Please enter your API key:"));
        process.stdout.write(chalk.cyan("> "));
    }
    ...
    if (provider === "openai") {
        ...
        console.log(chalk.gray("Browse available models at: https://openrouter.ai/models"));
        process.stdout.write(chalk.cyan("> "));
    ...
    try {
        ...
        // Test configuration
        console.log(chalk.yellow("Would you like to test this configuration now? (y/n)"));
        process.stdout.write(chalk.cyan("> "));
