...
import { checkProjectExists, extractProjectIdentifierFromTag } from "../../../shared/src/projects.js";
import { logError, logInfo, logSuccess, logWarning } from "../../../shared/src/logger.js";
import { Config } from "../config/config.js";
import { spawn } from "child_process";
import chalk from "chalk";

export async function handleChatEvent(event: NDKEvent, config: Config, ndk: NDK): Promise<void> {
    ...
    if (event.kind === 1111) {
        ...
        if (rootEvent.kind !== 11) {
            logWarning(`Root event ${rootEventTag[1]} is not kind:11 (found kind:${rootEvent.kind})`);
            return;
        }
    ...
    const projectTag = eventToProcess.tags.find(tag => tag[0] === "a" && tag[1]?.startsWith("31933:"));
