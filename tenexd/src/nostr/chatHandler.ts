...
export async function handleChatEvent(
    ...
    if (event.kind === EVENT_KINDS.THREAD_REPLY) {
        ...
        if (rootEvent.kind !== EVENT_KINDS.CHAT) {
            logWarning(
                `Root event ${rootEventTag[1]} is not kind:${EVENT_KINDS.CHAT} (found kind:${rootEvent.kind})`
            );
            return;
        }
    ...
    const projectTag = eventToProcess.tags.find(
        (tag) => tag[0] === "a" && tag[1]?.startsWith(`${EVENT_KINDS.PROJECT}:`)
    );

    if (!projectTag) {
        logWarning(`Chat event missing project 'a' tag for kind:${EVENT_KINDS.PROJECT}`);
        return;
    }
    ...
    try {
        ...
        logError(
            `Failed to handle chat event: ${err instanceof Error ? err.message : String(err)}`
        );
    }
}
