...
import { type NDKArticle, NDKPrivateKeySigner } from "@nostr-dev-kit/ndk";
...
async function fetchProjectFromNostr(naddr: string): Promise<ProjectEventData> {
    ...
    if (projectEvent.title) {
        ...
        projectTitle = titleTag?.[1] || projectName;
    }
...
async function fetchTemplateRepoUrl(template: string): Promise<string | undefined> {
    try {
...
async function createProjectDirectory(projectPath: string, repoUrl?: string): Promise<void> {
...
async function publishAgentProfiles(agentsConfig: AgentsJson, projectTitle: string): Promise<void> {
