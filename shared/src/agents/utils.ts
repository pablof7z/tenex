 ...
 */
export async function loadAgentsConfig(configPath: string): Promise<AgentsJson> {
    try {
        const content = await readFile(configPath, "utf-8");
        const rawConfig = JSON.parse(content) as LegacyAgentsJson;
 ...
 */
export async function saveAgentsConfig(configPath: string, agents: AgentsJson): Promise<void> {
    try {
 ...
 */
export function generateAgentAvatarUrl(displayName: string): string {
...
export async function getOrCreateAgentSigner(
    ...
    if (!agentEntry) {
        ...
        let projectNaddr: string | undefined;
        let projectAuthor: string | undefined;
        try {
