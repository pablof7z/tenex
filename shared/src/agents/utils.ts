...
export async function loadAgentsConfig(
	...
	try {
		...
		logWarning(
			`Failed to load agents config from ${configPath}: ${getErrorMessage(err)}`,
		);
		return {};
	}
}
			...
			if (agentEvent && agentEvent.kind === 4199) {
				...
				if (!agentsConfig[agentKey]) {
					...
					if (!agentSigner.privateKey) {
						throw new Error(
							`Failed to generate private key for agent ${agentName}`,
						);
					}
					const agentPrivateKeyBytes = Buffer.from(
